...
def dialog(num_choice=DEF_CHOICE):
    ...
    new = set(words).difference(set(DICT))
    # new dish on list.
    # Nice if we can get more new dishes
    if (len(new)):
        print(D.PROMOTE.format(dishes = new))
        promote()
        return
    ...
    if set(words) & set(MENU_MULTI):
        ...
        if set(words) & set(FORBIDDEN):
            ...
            print(D_MEM.format(dishes=entry))
